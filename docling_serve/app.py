...
from docling_serve.enhancement_store import get_enhancement_store, EnhancementOptions
...
def create_app():  # noqa: C901
    ...
    if docling_serve_settings.enable_ui:
        try:
            ...
            _log.warning(
                "Arabic Parser enable_ui is activated, but gradio is not installed. "
    ...
    # Track tasks currently undergoing enhancement so we don't duplicate work
    enhancement_processing: set[str] = set()
    ...
    async def _enque_source(
        ...
        # Store enhancement options for later use
        if conversion_request.options:
            enhancement_options = EnhancementOptions(
                enable_advanced_formula_enrichment=getattr(conversion_request.options, 'enable_advanced_formula_enrichment', False),
                enable_character_encoding_fix=getattr(conversion_request.options, 'enable_character_encoding_fix', False)
            )
            get_enhancement_store().set_options(task.task_id, enhancement_options)
    ...
    async def _enque_file(
        ...
        # Store enhancement options for later use
        enhancement_options = EnhancementOptions(
            enable_advanced_formula_enrichment=getattr(options, 'enable_advanced_formula_enrichment', False),
            enable_character_encoding_fix=getattr(options, 'enable_character_encoding_fix', False)
        )
        get_enhancement_store().set_options(task.task_id, enhancement_options)
    ...
    async def _maybe_start_enhancement(orchestrator: BaseOrchestrator, task_id: str, raw_status: str):
        """Kick off background enhancement if underlying conversion succeeded but enhancement not yet applied."""
        if raw_status != "success":
            return
        store = get_enhancement_store()
        if not store.get_options(task_id):
            return
        if task_id in enhancement_processing:
            return
        enhancement_processing.add(task_id)

        async def _run():
            try:
                task_result = await orchestrator.task_result(task_id=task_id)
                if task_result and isinstance(getattr(task_result, "result", None), ExportResult):
                    try:
                        from docling_serve.document_enhancement import DocumentProcessor
                        enhanced_result = task_result.result
                        enhancement_options = store.get_options(task_id)
                        if enhancement_options and hasattr(enhanced_result.content, 'json_content') and enhanced_result.content.json_content:
                            ...
                            new_doc = enhanced_result.content.json_content
                            try:
                        ...
                        _log.error(f"Background enhancement failure for {task_id}: {e}")
                store.remove_options(task_id)
            finally:
                enhancement_processing.discard(task_id)

        asyncio.create_task(_run())

    def _public_task_status(task_id: str, raw_status: str) -> str:
        """Return the task status exposed to clients.

        If a task has completed conversion (raw_status == 'success') but
        enhancement options are still stored for it, we keep reporting it as
        'started' so that clients don't treat it as fully finished until the
        enhancement phase (performed during response preparation) has run.
        """
        try:
            store = get_enhancement_store()
            # If options still exist we haven't yet executed enhancement logic
            if raw_status == "success" and store.get_options(task_id):
                return "started"
        except Exception:  # defensive: never break status endpoint
            pass
        return raw_status
    ...
    async def process_url_async(
        ...
        # Trigger background enhancement if needed
        await _maybe_start_enhancement(orchestrator, task.task_id, task.task_status)
        return TaskStatusResponse(
            task_id=task.task_id,
            task_status=_public_task_status(task.task_id, task.task_status),
