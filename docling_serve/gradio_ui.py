...
def wait_task_finish(auth: str, task_id: str, return_as_file: bool):
    ...
    while not task_finished:
        try:
            ...
            logger.warning(f"Status poll timeout ({consecutive_timeouts}/{MAX_TIMEOUTS}): {e}")
            if consecutive_timeouts >= MAX_TIMEOUTS:
                logger.error("Exceeded maximum consecutive status poll timeouts.")
                raise gr.Error("Status polling timed out.", print_exception=False)
            time.sleep(2)
            continue
    ...
    if conversion_sucess:
        try:
            ...
            logger.error(f"Result retrieval timed out: {e}")
            raise gr.Error("Timed out retrieving result.", print_exception=False)
        except Exception as e:
            logger.error(f"Error getting task result: {e}")
            raise gr.Error(f"Error getting task result: {e}", print_exception=False)

    raise gr.Error(
        f"Error getting task result, conversion finished with status: {task_status or 'unknown'}"
    )
